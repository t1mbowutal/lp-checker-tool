...
function kv(label, value) {
  return `<div class="card"><div class="kv"><b>${label}</b><span>${value}</span></div></div>`;
}
...
async function analyze(url) {
  ...
  try {
    ...
    const bullets = [
      ...
      kv("Hero section (H1 + CTA above the fold)", data.hasHero ? "Likely" : "Unclear"),
...
$("#exportBtn").addEventListener("click", () => {
  window.print(); // Uses print styles to export the report
});
