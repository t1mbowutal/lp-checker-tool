...
// src/lib/scoring.ts - updated scoring logic (ifm-v3)
...
export function scoreLanding(signals: Signals): ScoreResult {
  ...
  const known = <T>(v: T | null | undefined, fallback: T, reason?: string) => {
  ...
  const fields = known(signals.formFieldsCount, calibration.formPenalty.startFields, "FORM_FIELDS_UNKNOWN");
  let penalty = 0;
  ...
  let ux = 0;
  const lcp = known(signals.lcpMs, 3500, "LCP_UNKNOWN");
  const cls = known(signals.cls, 0.12, "CLS_UNKNOWN");
  const tti = known(signals.ttiMs, 3500, "TTI_UNKNOWN");
